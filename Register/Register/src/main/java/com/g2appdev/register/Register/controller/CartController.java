...
public class CartController {
 ...
 // GET cart of a specific user with only active cart items
    @GetMapping("/user/{userId}/active")
    public ResponseEntity<CartDTO> getActiveCartItemsByUserId(@PathVariable int userId) {
        try {
            // Fetch the cart for the user
            CartEntity cart = cartService.createOrGetCart(userId);

            // Filter active cart items and map to DTO
            List<CartItemDTO> activeCartItems = cart.getCartItems().stream()
                .filter(cartItem -> "active".equalsIgnoreCase(cartItem.getStatus())) // Only active items
                .map(cartItem -> {
                    ...
                    itemDTO.setMenuItemId(menuItem.getMenuItemID());  // Map menuItemId
                    itemDTO.setName(menuItem.getName());               // Map name
                    itemDTO.setCategory(menuItem.getCategory());       // Map category

                    return itemDTO;
                })
                .collect(Collectors.toList());

            // Create CartDTO response
            CartDTO cartDTO = new CartDTO();
            cartDTO.setCartId(cart.getCartId());
            cartDTO.setCartItems(activeCartItems);

            return ResponseEntity.ok(cartDTO);
        } catch (Exception e) {
            System.err.println("Error fetching active cart items: " + e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(null);
        }
    }
