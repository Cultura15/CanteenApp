    ...
    useEffect(() => {
        const fetchMenuItem = async () => {
            try {
                ...
                const eggItem = response.data.find(item => item.name === "HotCoffee"); // Replace with your actual condition
                
                if (eggItem) {
                    console.log('Hot Coffee item:', eggItem); // Log the found item
                    setMenuItem(eggItem); // Set the fetched item
                } else {
                    console.warn('Hot Coffee not found in the fetched data.'); // Warn if not found
                }
            } catch (error) {
