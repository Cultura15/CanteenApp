    ...
    useEffect(() => {
        const fetchMenuItem = async () => {
            try {
                ...
                const eggItem = response.data.find(item => item.name === "Sandwhich"); // Replace with your actual condition
                
                if (eggItem) {
                    console.log('Sandwhich item:', eggItem); // Log the found item
                    setMenuItem(eggItem); // Set the fetched item
                } else {
                    console.warn('Sandwhich not found in the fetched data.'); // Warn if not found
                }
            } catch (error) {
    ...
    return (
        <div className="sunny-side-up-page">
            ...
            <div className="description">
                ...
                <p>Sandwich, in its basic form, slices of meat, cheese, or other
                     food placed between two slices of bread. Although this 
                     mode of consumption must be as old as meat and bread, the 
                     name was adopted only in the 18th century for John Montagu, 
                     4th earl of Sandwich.</p>       
            </div>
