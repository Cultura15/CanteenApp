    ...
    useEffect(() => {
        const fetchMenuItem = async () => {
            try {
                ...
                const eggItem = response.data.find(item => item.name === "Pizza"); // Replace with your actual condition
                
                if (eggItem) {
                    console.log('Pizza item:', eggItem); // Log the found item
                    setMenuItem(eggItem); // Set the fetched item
                } else {
                    console.warn('Pizza not found in the fetched data.'); // Warn if not found
                }
            } catch (error) {
    ...
    return (
        <div className="sunny-side-up-page">
            ...
            <div className="description">
                ...
                <p> Pizza[a][1] is an Italian dish typically consisting of a flat base of leavened wheat-based dough topped with tomato, cheese, and
                     other ingredients, baked at a high temperature, traditionally in a wood-fired oven.</p>       
            </div>
