...
const Adobo = () => {
    ...
    const [isFlying, setIsFlying] = useState(false); // State to control flying animation
    const [isImageVisible, setIsImageVisible] = useState(false); // State for visibility
    const [imagePosition, setImagePosition] = useState({ x: 0, y: 0 }); // Position of the image
    ...
    useEffect(() => {
        const fetchMenuItem = async () => {
            try {
                const response = await axios.get('http://localhost:8080/api/menu/get');
                const eggItem = response.data.find(item => item.name === "Adobo");
                if (eggItem) {
                    setMenuItem(eggItem);
                }
            } catch (error) {
    ...
    const addToCart = async () => {
        ...
        // Calculate the position of the "Add to Cart" button relative to the viewport
        const button = document.querySelector('.add-to-cart');
        const buttonRect = button.getBoundingClientRect();
    
        setImagePosition({
            x: buttonRect.x + buttonRect.width / 2, // Start position (center of the button)
            y: buttonRect.y + buttonRect.height / 2, // Start position (center of the button)
        });
    
        setIsFlying(true); // Trigger the flying effect
        setIsImageVisible(true); // Make the image visible
    
        setTimeout(() => {
            setIsFlying(false);  // Stop the flying effect
            setIsImageVisible(false); // Make the image disappear
        }, 2000); // 2 seconds
        ...
        <div className="sunny-side-up-page">
            ...
            <div className="food-detail-container">
                <div className="image-container">
                    <img src="/assets/adobo.png" alt="Chicken Adobo" />
                </div>
            ...
            <div className="description">
                ...
                <p>Chicken adobo from Spanish adobar: "marinade," "sauce" or "seasoning" is a popular Filipino dish.</p>
            </div>
            ...
            <div className={`flying-image ${isFlying ? 'fly' : ''}`} style={{
...
.flying-image img {
    width: 50px;  /* Set a smaller size */
    height: 50px; /* Set a smaller size */
}

.flying-image.fly {
    /* Adjust the distance of translation to make it go all the way to the header */
    transform: translateY(-600px) translateX(-500px); /* Ensure this value is sufficient to reach the top header */
    opacity: 1;  /* Keep it visible during flight */
}

/* Add a delay and fade the image after it's been visible for 5 seconds */
.flying-image.show {
    opacity: 1;
    visibility: visible;
    transition: opacity 0s 3s; /* Keep it visible for 3 seconds */
}

.flying-image.fly.show {
    opacity: 0;
    visibility: hidden; /* Make it hidden after 5 seconds (3 seconds after start of flying animation) */
    transition: opacity 2s 2s; /* This ensures the image fades out after 5 seconds total */
}
